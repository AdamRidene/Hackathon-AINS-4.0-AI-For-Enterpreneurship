...
function buildSteps(trace, value, question, lang) {
  ...
  if (has("probe_answer:")) {
    steps.push({ icon: "📥", label: ar ? "إجابة السبر مُسجَّلة" : "Réponse à la sonde enregistrée", detail: valShort, node: "ingest" });
  } else if (has("answer:")) {
    steps.push({ icon: "📥", label: ar ? "إجابة مُستلمة" : "Réponse reçue", detail: valShort, node: "ingest" });
  }

  // 2 — LLM probe decision
  if (has("probe_emitted:")) {
    steps.push({ icon: "🧠", label: ar ? "تحليل النص الحر (IA)" : "Analyse IA du texte libre", node: "generate_probe" });
    steps.push({ icon: "🤖", label: ar ? "سبر مُولَّد" : "Sonde générée", detail: qText, node: "generate_probe", accent: true });
  } else if (trace.includes("probe_declined")) {
    steps.push({ icon: "🧠", label: ar ? "تحليل IA: الإجابة كافية" : "Analyse IA : réponse jugée suffisante", node: "generate_probe" });
  }

  // 3 — outcome
  if (trace.includes("serve_pending_probe")) {
    steps.push({ icon: "🤖", label: ar ? "تقديم سبر مُعلَّق" : "Sonde en attente servie", detail: qText, node: "finalize", accent: true });
  } else if (trace.includes("deterministic_next")) {
    steps.push({ icon: "➡️", label: ar ? "السؤال الحتمي التالي" : "Question déterministe suivante", detail: qText, node: "finalize" });
  } else if (trace.includes("intake_complete")) {
    steps.push({ icon: "✅", label: ar ? "اكتمل الاستبيان — انطلاق التدقيق" : "Intake terminé — audit lancé", node: "finalize" });
  }
